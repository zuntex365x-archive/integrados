...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(Inv_proceso_r))
        ...
        Me.Button3.Location = New System.Drawing.Point(944, 8)
        ...
        Me.Name = "Inv_proceso_r"
        Me.ShowInTaskbar = False
        Me.StartPosition = System.Windows.Forms.FormStartPosition.CenterScreen
        Me.Text = "Inventario en Proceso (Resumido)"
...
#End Region
    ...
    Private Sub setea_grid()
        Try
            ...
            fg.Cols(4).Caption = "CLIENTE"
            fg.Cols(5).Caption = "YARDAS"
            fg.Cols(6).Caption = "LIBRAS"
            fg.Cols(7).Caption = "COSTO L."
            fg.Cols(8).Caption = "COSTO TOT."
            fg.Cols(9).Caption = "CODIGO"
            fg.Cols(10).Caption = "DESCRIPCION DE LA TELA"
            fg.Cols(11).Caption = "FPO"
            fg.Cols(5).DataType = GetType(Decimal)
            fg.Cols(5).Format = h
    ...
    Private Sub llena_fg()
        ...
        ' strsql = "SELECT DMOVTO.BATCH ,ROLLOS.KNIT,ROLLOS.COLOR, ROLLOS.CLIENTE, SUM(DMOVTO.YARDAS) AS YARDA, (SUM(ROLLOS.PESO)/ SUM(ROLLOS.YARDAS_I)) * SUM(DMOVTO.YARDAS) AS LIBRA , SUM(ROLLOS.PESO)AS TOTAL, SUM(ROLLOS.PESO) AS TOTALON,ROLLOS.CODIGO,DESCRIPCION,DMOVTO.FPO  FROM DMOVTO LEFT JOIN CORTES ON CORTES.CORTE = DMOVTO.CORTE LEFT JOIN ROLLOS ON DMOVTO.BATCH = ROLLOS.BATCH AND DMOVTO.ROLLO = ROLLOS.ROLLO LEFT JOIN CODIGO_TELA ON CODIGO_TELA.CODIGO = ROLLOS.CODIGO WHERE DMOVTO.TMOVTO > 3 AND EXPORTADO <> 'S' AND (SELECT COUNT(PRENDAS) FROM PROD_DIARIA WHERE PROD_DIARIA.CORTE = CORTES.CORTE) = 0 and FCORTE < = '" & fechas & "' GROUP BY DMOVTO.BATCH, ROLLOS.KNIT,ROLLOS.COLOR, ROLLOS.CLIENTE, DMOVTO.FPO, ROLLOS.CODIGO, DESCRIPCION  ORDER BY DMOVTO.BATCH"
        strsql = "SELECT DMOVTO.CORTE,FCORTE, FPROD,DMOVTO.BATCH, DMOVTO.CPO, DMOVTO.CLIENTE ,ROLLOS.KNIT, ROLLOS.COLOR, SUM(DMOVTO.YARDAS) AS YARDA, (SUM(ROLLOS.PESO)/ SUM(ROLLOS.YARDAS_I)) * SUM(DMOVTO.YARDAS) AS LIBRA, DMOVTO.FPO ,SUM(ROLLOS.PESO)AS PTOTAL,TOTAL,SECCION,SUM(DMOVTO.PRECIO*DMOVTO.LIBRAS) AS CTOTAL,ROLLOS.CODIGO,DESCRIPCION FROM DMOVTO LEFT JOIN ROLLOS ON DMOVTO.BATCH =ROLLOS.BATCH AND DMOVTO.ROLLO = ROLLOS.ROLLO  LEFT JOIN CORTES ON DMOVTO.CORTE = CORTES.CORTE  LEFT JOIN CODIGO_TELA ON CODIGO_TELA.CODIGO = ROLLOS.CODIGO WHERE CORTES.CORTE = DMOVTO.CORTE AND DMOVTO.TMOVTO > 3 AND DMOVTO.BATCH = ROLLOS.BATCH AND DMOVTO.ROLLO = ROLLOS.ROLLO AND CORTES.EXPORTADO <> 'S' AND (SELECT COUNT(PRENDAS) FROM PROD_DIARIA WHERE PROD_DIARIA.CORTE= CORTES.CORTE) = 0  GROUP BY DMOVTO.CORTE,FCORTE,FPROD,DMOVTO.BATCH,DMOVTO.CPO, DMOVTO.CLIENTE, ROLLOS.KNIT, ROLLOS.COLOR, DMOVTO.FPO ,TOTAL,SECCION,ROLLOS.CODIGO,DESCRIPCION ORDER BY DMOVTO.CORTE "
        ...
        For Each dr In dt.Rows
            ...
            knit = dr("KNIT")
            colores = dr("COLOR")
            f = fg.FindRow(batch + knit, 1, 0, False)
            If f < 1 Then
                fg(l, 1) = batch
                fg(l, 2) = knit
                fg(l, 3) = colores
                fg(l, 4) = dr("CLIENTE")
                fg(l, 9) = dr("CODIGO")
                fg(l, 10) = dr("DESCRIPCION")
                fg(l, 0) = batch + knit
                f = l
                l = l + 1
            End If

            fg(f, 5) = fg(f, 5) + dr("YARDA")
            fg(f, 6) = fg(f, 6) + dr("LIBRA")

            fpo = "FPO = '" & dr("FPO") & "' AND KNIT = '" & knit & "' AND COLOR = '" & colores & "'"
            Try
                dw = ct.Select(fpo)
                dj = dw(0)
                fg(f, 7) = Format(dj("PRECIO"), g)
                fg(f, 8) = fg(f, 8) + (dj("PRECIO") * dr("LIBRA"))
            Catch
                Try
                    fg(f, 7) = (dr("CTOTAL") / dr("LIBRA"))
                    fg(f, 8) = fg(f, 8) + dr("CTOTAL")
                Catch
                End Try
            End Try
    ...
    Private Sub busca_costos()
        ...
        For i = 1 To fg.Rows.Count - 1
            If fg(i, 7) = 0 Then
                dw = dt.Select("KNIT = '" & fg(i, 2) & "' AND COLOR = '" & fg(i, 3) & "'")
                If dw.Length > 0 Then
                    dr = dw(0)
                    fg(i, 7) = dr("PRECIO")
    ...
    Private Sub Print_fg()
        ...
        mal = obj.nombre + "  Inventario en Proceso Resumido al " + fecha
