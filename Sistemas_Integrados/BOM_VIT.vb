...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(BOM_VIT))
        ...
        Me.criterio.Items.AddRange(New Object() {"ESTILO", "COLOR", "FECHA", "CODIGO", "USUARIO", "TIPO"})
        ...
        Me.Text = "Vitácora de Cambios al BOM"
...
#End Region
    ...
    Private Sub criterio_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles criterio.SelectedIndexChanged
        ...
        Select Case criterio.SelectedIndex
            Case 0
                strsql = "SELECT DISTINCT ESTILO FROM MAT_EST_COL_V  ORDER BY ESTILO"
            Case 1
                strsql = "SELECT DISTINCT COLOR FROM MAT_EST_COL_V  ORDER BY COLOR"
            Case 2
                strsql = "SELECT  DISTINCT(CONVERT(nvarchar(10), FECHA, 20)) AS FECHA FROM MAT_EST_COL_V "
            Case 3
                strsql = "SELECT DISTINCT CODIGO FROM MAT_EST_COL_V  "
            Case 4
                strsql = "SELECT DISTINCT USUARIO FROM MAT_EST_COL_V  "
            Case 5
                strsql = "SELECT DISTINCT TM_DESCRIPCION FROM MAT_EST_COL_V1 "
        End Select
        llena_combos(nombre, strsql, criterio.Text)
        If criterio.SelectedIndex = 5 Then
            llena_combos(nombre, strsql, "TM_DESCRIPCION")
        End If
    End Sub
    ...
    Private Sub procesa()
        Dim busca As String = "MAT_EST_COL_V." & criterio.Text & " =  '" & nombre.Text & "'"
        If criterio.SelectedIndex = 2 Then
            busca = " YEAR(FECHA)  = '" & Mid(nombre.Text, 1, 4) & "' AND MONTH(FECHA) = '" & Mid(nombre.Text, 6, 2) & "' AND DAY(FECHA) = '" & Mid(nombre.Text, 9, 2) & "'"
        End If
        If criterio.Text = "TIPO" Then
            busca = "MAT_EST_COL_V1.TM_DESCRIPCION  =  '" & nombre.Text & "'"
        End If
        strsql = "SELECT * FROM MAT_EST_COL_V LEFT JOIN MAT_EST_COL_V1 ON MAT_EST_COL_V.TM = MAT_EST_COL_V1.TM WHERE " & busca
        llena_grid()
    End Sub
