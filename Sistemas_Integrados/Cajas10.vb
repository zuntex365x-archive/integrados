...
#End Region
    ...
    '    Dim dr As DataRow
    '    Dim dv As DataView = pr.DefaultView
    '    dv.Sort = "SECCION ASC"
    '    pr = dv.ToTable()
    ...
    '        seccion = dr("SECCION")
    '        prendas = dr("PRENDA")
    '        indice = fg.Cols.IndexOf("TEXSUN " + seccion)
    '        If indice < 1 Then
    ...
    Private Sub produccion(ByVal cnn As SqlClient.SqlConnection)
        ...
        strSQL = "SELECT CONVERT (date,FECHA) AS FECHA , SUM(UNIDADES) AS PRENDA, SECCION FROM CAJAS04 LEFT JOIN CORTES ON CAJAS04.CORTE = CORTES.CORTE WHERE MONTH(FECHA) = '" & mes & "' AND YEAR(FECHA) ='" & ano & "' AND (SECCION LIKE 'TEX%' OR SECCION LIKE 'INNO%')  GROUP BY SECCION,CONVERT (date,FECHA) ORDER BY SECCION"
        dt = New DataTable
        Dim dr As DataRow
    ...
    '    Dim em() As String
    '    Dim strSQL As String
    '    Dim util As New Lib_SI.util
    '    em = e.Split("|")
    '    pr = New DataTable
    '    For i = 0 To 2
    '        dt = New DataTable
    '        strSQL = "SELECT CONVERT (date,CAJAS04.FECHA) AS FECHA , SUM(CAJAS01.UNIDADES) AS PRENDA, RIGHT(CAJAS01.SECCION,1) AS SECCION FROM CAJAS04 LEFT JOIN CAJAS01 ON CAJAS04.CAJA = CAJAS01.CAJA AND CAJAS01.CORTE = CAJAS04.CORTE AND CAJAS01.TALLA = CAJAS04.TALLA AND CAJAS04.TIPO = CAJAS01.TIPO LEFT JOIN CORTES ON CAJAS04.CORTE = CORTES.CORTE WHERE MONTH(CAJAS04.FECHA) = '" & mes & "' AND YEAR(CAJAS04.FECHA) ='" & ano & "' AND CAJAS01.SECCION LIKE 'TEX%' AND TIPO_SEG NOT IN ('3','8','12') GROUP BY CAJAS01.SECCION,CONVERT (date,CAJAS04.FECHA) ORDER BY RIGHT(CAJAS01.SECCION,1)"
    '        cnn.ConnectionString = util.con_string(i)
    '        llena_tablas_e(dt, strSQL, cnn)
    '        pr.Merge(dt)
    '    Next
    ...
    Private Sub fg_Click(sender As System.Object, e As System.EventArgs) Handles fg.Click
        ...
        If fg.ColSel > 1 And fg.ColSel < fg.Cols.Count Then
            If fg(fg.RowSel, fg.ColSel) > 0 Then
                strsql = "SELECT * FROM CAJAS04 LEFT JOIN CORTES ON CAJAS04.CORTE = CORTES.CORTE inner JOIN CAJAS01 ON CAJAS01.CAJA = CAJAS04.CAJA WHERE CONVERT(date,CAJAS04.FECHA) = '" & fecha & "' AND CAJAS01.SECCION = '" & seccion & "'"
