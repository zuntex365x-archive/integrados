...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(Wip3))
...
#End Region
    ...
    Private Sub setea_fg()
        ...
        fg.Cols(10).Name = "STATUS"
        fg.Cols(11).Name = "OFFER D."
        fg.Cols(12).Name = "PLAN"
        fg.Cols(13).Name = "ACTUAL"
        fg.Cols(14).Name = "ACT.PCS"
        fg.Cols(15).Name = "% AC-PL"
        fg.Cols(30).Visible = False
        fg.Cols(46).Name = "  X-CHAR"
        fg.Cols(47).Name = "PROGRAM"
        fg.Cols(48).Name = "DESTINATION"
        For index = 1 To fg.Cols.Count - 1
            ...
            End If
        Next
        fg.Cols(46).Visible = False
        fg.Cols(47).Visible = True
        fg.Cols(46).Visible = True
        fg.Cols(48).Visible = True
        fg.Cols(46).Width = 70
        fg.Cols(47).Width = 120
        fg.Cols(46).Width = 150
        fg.Cols(48).Width = 180
        fg.Cols(46).AllowMerging = True
        fg.Cols(47).AllowMerging = True
        fg.Cols(48).AllowMerging = True
        forma_fecha(1)
        forma_entero(6)
        forma_fecha(7)
        forma_entero(8)
        ...
        fg.Cols(25).Width = 50
        fg.Cols(25).TextAlign = TextAlignEnum.RightCenter
        ...
        rng1 = fg.GetCellRange(0, 10, 0, 11)
        rng1.Data = "TRIMS"
        rng1.Style = fg.Styles("c1")


        rng1 = fg.GetCellRange(1, 10, 1, 11)
        rng1.Style = fg.Styles("c2")
        ...
        rng1 = fg.GetCellRange(0, 46, 1, 46)
        rng1.Style = fg.Styles("c1")
        forma_fecha(46)
    End Sub
    ...
    Private Sub llena_fg()
        ...
        For Each dr In cpo.Rows
            ...
            For j = 0 To foundrows.GetUpperBound(0)
                ...
                End Try
                Try
                    ofecha = fpo.Compute("MAX(OFECHA)", "CODIGO = '" & knit & "' AND COLOR = '" & colo & "' AND CPO = '" & cpos & "'")
                    If CDate(ofecha) > fechao Then
                        fechao = CDate(ofecha)
                    End If
                Catch
                End Try
    ...
    Private Sub estilos_linea(ByRef l As Integer, ByVal ahora As String)
        ...
        End If
    End Sub
    Private Sub llena_trims()
        strSQl = "SELECT DISTINCT CPO_D.CPO,(SELECT TOP 1 FECHA_ENTREGA FROM PEDIDOS WHERE PEDIDOS.CPO = CPO_D.CPO ORDER BY FECHA_ENTREGA DESC) AS FECHA_T ,(SELECT TOP 1 STATUS FROM PEDIDOS WHERE PEDIDOS.CPO = CPO_D.CPO ORDER BY STATUS) AS ESTADO FROM CPO_D LEFT JOIN CPO ON CPO_D.CPO = CPO.CPO WHERE CLIENTE = '" & C4.Text & "' AND ESTADO = 'A' ORDER BY CPO_D.CPO"
        llena_tablas(tr, strSQl, cnn)
    End Sub
    ...
    Private Sub exportacion(ByVal c As Integer)
        ...
        Dim por As Decimal
        Dim total As Integer
        strSQl = "SELECT CPO,ESTILO,COLOR,SUM(XS+S+M+L+XL+XL2+XL3+XL4+XL5+XL6) AS TOTAL FROM PACKING_D WHERE CLIENTE = '" & C4.Text & "' AND CPO+ESTILO+COLOR IN (SELECT CPO+ESTILO+COLOR FROM CPO_D WHERE ESTADO = 'A') AND TIPO = 'Pri'  GROUP BY CPO,ESTILO,COLOR ORDER BY CPO,ESTILO,COLOR"
        llena_tablas(dt, strSQl, cnn)
        For Each dr In dt.Rows
            total = 0
            busca = Trim(dr("CPO")) + Trim(dr("ESTILO")) + Trim(dr("COLOR"))
    ...
    Private Sub controla_procesos()
        ...
        For i = 2 To 8
            If ru(i) > 0 And i <> 6 And i <> 7 Then
        ...
        Try
            If Trim(fg(f, 2)) <> "" Then
                If (c > 6 And c < 10) Then
                    detalle_fpo(f)
                ElseIf (c > 9 And c < 12) Then
                    Dim forma As New Pedidos_cpo
                    forma.Cpo.Text = fg(f, 2)
                    forma.Cliente.Text = C4.Text
                    forma.Fecha.Text = fg(f, 1)
                    forma.ShowDialog()
                ElseIf (c > 11 And c < 16) Then
    ...
    Private Function busca_tabla(ByVal dt As DataTable, ByVal busca As String, ByRef dr As DataRow)
        Dim dd As DataRow()
        dd = dt.Select(busca)
        If dd.Length > 0 Then
            dr = dd(0)
