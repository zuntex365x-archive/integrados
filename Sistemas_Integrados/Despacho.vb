...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.Cortes.Location = New System.Drawing.Point(128, 8)
        Me.Cortes.Name = "Cortes"
        Me.Cortes.Size = New System.Drawing.Size(115, 24)
        ...
        Me.fg.Size = New System.Drawing.Size(968, 584)
...
#End Region
    ...
    Private Sub llena_Grid()
        Dim dr As DataRow
        Dim i As Integer
        Dim strSQL As String = "SELECT REQ_D.*, KNIT,COLOR,FPO,CPO,CLIENTE,BARRA,COSTO FROM REQ_D,ROLLOS WHERE REQ_D.BATCH = ROLLOS.BATCH AND REQ_D.ROLLO = ROLLOS.ROLLO AND REQ_D.CORTE = '" & Cortes.Text & "' AND REQ_D.DESPACHADO = 'N' ORDER BY REQ_D.BATCH, REQ_D.ROLLO"
    ...
    Private Sub llena_combos(ByRef c As ComboBox, ByRef ok As Boolean)
        Dim dt As New DataTable
        Dim dr As DataRow
        Dim cnn1 As New SqlClient.SqlConnection
        Dim strsql As String = "SELECT DISTINCT CORTE FROM DESP_T2 WHERE EMPRESA = '" & obj.numero & "'"
        c.Items.Clear()
        ok = True
        cnn1.ConnectionString = lector_dc()
        llena_tablas_e(dt, strsql, cnn1)
        For Each dr In dt.Rows
            c.Items.Add(dr("CORTE"))
        Next
    ...
    Private Sub procesa_descarga()
        Try
            ...
            Dim i As Integer
            Dim cnn1 As New SqlClient.SqlConnection
            Dim strsql As String = "SELECT * FROM DESP_T2 WHERE CORTE = '" & Cortes.Text & "' AND EMPRESA = '" & obj.numero & "'"
            cnn1.ConnectionString = lector_dc()
            llena_tablas_e(dt, strsql, cnn1)
            For Each dr In dt.Rows
                i = fg.FindRow(dr("CODIGO"), 1, 8, True)
                If i > 0 Then
                    fg(i, 7) = True
                End If
            Next
    ...
    Private Sub elimina_lector()
        ...
        Try
            ...
            strSQL = "DELETE DESP_T2 WHERE CORTE = '" & Cortes.Text & "'"
            comando.CommandText = strSQL
            afectados = comando.ExecuteNonQuery()

            strSQL = "DELETE DESP_T1 WHERE CORTE = '" & Cortes.Text & "'"
            comando.CommandText = strSQL
            afectados = comando.ExecuteNonQuery()
