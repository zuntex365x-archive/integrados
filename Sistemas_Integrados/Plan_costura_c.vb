...
Public Class Plan_costura_c
    ...
    Private Sub vacaciones()
        strsql = "SELECT * FROM SECCIONES_VAC WHERE FECHA_FINAL > = '" & Format(Today, "yyyy-MM-dd") & "' ORDER BY SECCION,FECHA_INICIAL"
    ...
    Private Sub calcula_fecha_produccion()
        ...
        For i = 1 To fg.Rows.Count - 1
            If sec <> fg(i, "SEC") Then
                ...
                prd = Today
                ajusta_fecha_produccion(fg(i, "SEC"), Today, 0, prd)
            End If
            If fg(i, "PRO") < fg(i, "PRE") Then
                ...
                dias = pr / nmeta
                ajusta_fecha_produccion(fg(i, "SEC"), fg(i, "FCS"), dias, prd)
    ...
    Private Sub actualiza_plan()
        ...
        Try
            For i = 1 To fg.Rows.Count - 1
                ...
                If fg(i, "TIP") = "F" Then
                    f_prod = Format(fg(i, "FCS"), "yyyy-MM-dd")
                    strsql = "UPDATE PLAN_COSTURA_CP SET  PRIORIDAD = '" & fg(i, "PRI") & "' , SECCION = '" & fg(i, "SEC") & "' , COMENTARIOS = '" & fg(i, "COM") & "', F_PROD = '" & f_prod & "' WHERE CPO = '" & fg(i, "CPO") & "' AND ESTILO = '" & fg(i, "EST") & "' AND COLOR = '" & fg(i, "COL") & "'"
                    comando.CommandText = strsql
                    afectados = comando.ExecuteNonQuery()

                    If afectados = 0 Then
                        strsql = "INSERT INTO PLAN_COSTURA_CP (CPO,ESTILO,COLOR,PRIORIDAD,SECCION,COMENTARIOS,F_PROD) " & _
                               "VALUES ( '" & fg(i, "CPO") & "','" & _
                                   fg(i, "EST") & "','" & _
                                   fg(i, "COL") & "','" & _
                                   fg(i, "PRI") & "','" & _
    ...
    Private Sub actualizado()
        ...
        For Each dr In dt.Rows
            Label2.Text = Trim(dr("USUARIO")) & " al " & Format(dr("FECHA"), "dd-MMM-yyyy  hh:mm:ss")
        Next
        Label2.Visible = True
        Label3.Visible = True
    End Sub
