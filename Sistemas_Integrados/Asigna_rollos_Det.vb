...
Public Class Asigna_rollos_Det
...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(Asigna_rollos_Det))
        ...
        Me.fg.BackColor = System.Drawing.Color.FromArgb(CType(CType(244, Byte), Integer), CType(CType(254, Byte), Integer), CType(CType(207, Byte), Integer))
        ...
        Me.Co.Location = New System.Drawing.Point(8, 608)
        Me.Co.Name = "Co"
        ...
        Me.Co.Size = New System.Drawing.Size(984, 96)
        ...
        'Label4
        '
        Me.Label4.Location = New System.Drawing.Point(8, 592)
        ...
        Me.Label5.Location = New System.Drawing.Point(8, 568)
        ...
        Me.ClientSize = New System.Drawing.Size(1002, 704)
...
#End Region
    ...
    Private Sub llena_ta()
        ...
        Dim dr As DataRow
        Dim strSQL As String = "SELECT KNIT_TELA, COLOR_TELA, LIBRAS FROM CPO_EST_COL WHERE ESTILO_CPO = '" & Estilos.Text & "' AND COLOR_CPO = '" & colors.Text & "'"
        llena_tablas(dt, strSQL, cnn)
        For Each dr In dt.Rows
            lineas = Ta.Rows.Count
            Ta.Rows.Count = Ta.Rows.Count + 1
            Ta(lineas, 1) = dr("KNIT_TELA")
    ...
    Private Sub busca_fpos(ByVal knit_T As String, ByVal color_t As String, ByVal l As Integer)
        Dim dt As New DataTable()
        Dim dr As DataRow
        Dim strSQL As String = "SELECT * FROM FPO WHERE KNIT = '" & knit_T & "' AND COLOR = '" & color_t & "' AND STATUS = 'ABIERTO'"
        llena_tablas(dt, strSQL, cnn)
        For Each dr In dt.Rows
    ...
    Private Sub llena_fg()
        ...
        For i = 1 To Ta.Rows.Count - 1
            busca_telas(Ta(i, 1), Ta(i, 2), i)
            busca_fpos(Ta(i, 1), Ta(i, 2), i)
            Ta(i, 7) = Ta(i, 4) - Ta(i, 5) - Ta(i, 6)
        Next
        fg.Sort(SortFlags.Ascending, 1)
    End Sub

    Private Sub busca_telas(ByVal knit_T As String, ByVal color_t As String, ByVal l As Integer)
        Dim dt As New DataTable()
        ...
        Dim batch As String
        Dim i As Integer
        Dim strSQL As String = "SELECT ROLLOS.* , ((ROLLOS.PESO/ROLLOS.YARDAS_I) * ROLLOS.YARDAS) AS LIBRAS FROM ROLLOS WHERE KNIT = '" & knit_T & "' AND COLOR = '" & color_t & "' AND ACTIVO <> 'N' AND YARDAS > 0"
        llena_tablas(dt, strSQL, cnn)
        ...
        For Each dr In dt.Rows
            ...
            res = ap.Select("BATCH = '" & batch & "' AND CODIGO = '" & codigo & "' AND ESTADO = 'PENDIENTE'")
            If res.Length > 0 Then
                fg.SetCellStyle(lineas, 1, fg.Styles("amarillo"))
            End If
            res = ap.Select("BATCH = '" & batch & "' AND CODIGO = '" & codigo & "' AND ESTADO = 'RECHAZADO'")
            If res.Length > 0 Then
    ...
    Private Sub Tela_usada()
        Try
            ...
            Dim r As Integer
            Dim strSQL As String = "SELECT(SELECT KNIT FROM ROLLOS WHERE DMOVTO.BATCH = ROLLOS.BATCH AND DMOVTO.ROLLO = ROLLO) AS KNIT, (SELECT COLOR FROM ROLLOS WHERE DMOVTO.BATCH = ROLLOS.BATCH AND DMOVTO.ROLLO = ROLLO) AS COLOR, LIBRAS FROM DMOVTO WHERE TMOVTO ='4' AND CORTE IN (SELECT CORTE FROM CORTES WHERE CPO = '" & cpo & "' AND ESTILO = '" & estilo & "' AND COLOR = '" & colo & "') ORDER BY KNIT, COLOR"
            llena_tablas(dt, strSQL, cnn)
            For Each dr In dt.Rows
                knit = dr("KNIT")
                color = dr("COLOR")
