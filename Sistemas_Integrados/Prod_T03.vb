...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(PROD_T03))
        ...
        Me.corte.Location = New System.Drawing.Point(200, 30)
        Me.corte.Name = "corte"
        ...
        Me.cajad.Location = New System.Drawing.Point(199, 68)
        Me.cajad.Name = "cajad"
        ...
        Me.unidn.Location = New System.Drawing.Point(199, 31)
        ...
        Me.tallad.Location = New System.Drawing.Point(199, 139)
        ...
        Me.fg.Location = New System.Drawing.Point(23, 304)
        Me.fg.Name = "fg"
        Me.fg.Rows.DefaultSize = 21
        Me.fg.Size = New System.Drawing.Size(962, 357)
        ...
        Me.GroupBox1.Size = New System.Drawing.Size(468, 211)
        ...
        Me.tipod.Location = New System.Drawing.Point(199, 103)
        Me.tipod.Name = "tipod"
        ...
        Me.GroupBox2.Size = New System.Drawing.Size(468, 211)
...
#End Region
    Private Sub Cortes_status(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        AddHandler unidn.KeyPress, AddressOf keypressed4
    ...
    Private Sub setea_fg()
        ...
        fg.Rows.Fixed = 1
        fg.Rows(0).Height = 30
        llena_combos(corte, "SELECT DISTINCT CORTE FROM PROD_TER01 WHERE ESTADO = 'A' ORDER BY CORTE", "CORTE")
    End Sub
    Private Sub si_Click(sender As System.Object, e As System.EventArgs)
        llena_corte()
    End Sub

    Private Sub llena_corte()
        llena_tablas(co, "SELECT PROD_TER01.*, 0 AS ORDEN FROM PROD_TER01 WHERE CORTE = '" & corte.Text & "'", cnn)
        llena_combos(cajad, "SELECT DISTINCT CAJA FROM PROD_TER01 WHERE CORTE = '" & corte.Text & "' AND UNIDADES > 0 ORDER BY CAJA", "CAJA")
        If co.Rows.Count > 0 Then
            dr = co.Rows(0)
            pd.cliente = dr("CLIENTE")
            pd.estado = dr("ESTADO")
            pd.escala = dr("ESCALA")
            pd.orden = dr("ORDEN")
        End If
    ...
    Private Sub keypressed3(ByVal o As [Object], ByVal e As KeyPressEventArgs)
        If e.KeyChar = Microsoft.VisualBasic.ChrW(13) Then
            e.Handled = True
            unid.Text = saldo_caja(co, cajad.Text, tipod.Text, tallad.Text, orden)
            unidn.Focus()
        End If
    End Sub 'keypressed
    ...
    Private Sub cajad_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles cajad.SelectedIndexChanged
        llena_combos(tipod, "SELECT DISTINCT TIPO FROM PROD_TER01 WHERE CAJA = '" & cajad.Text & "' ORDER BY TIPO", "TIPO")
    End Sub
    ...
    Private Sub sigue_Click(sender As System.Object, e As System.EventArgs) Handles sigue.Click
        ...
        If Not ok Then
            ...
            Exit Sub
        End If
        ok = modifica_caja(co, cajad.Text, tipod.Text, tallad.Text, unidn.Text, orden, unid.Text)
        unidn.Text = 0
        cajad.Focus()
    End Sub
    ...
    Private Sub agrega_fg(ByRef ok As Boolean)
        Dim l As Integer
        ok = True
        l = fg.FindRow(cajad.Text + tallad.Text, 1, 0, False)
        If l > -1 Then
            ok = False
            Exit Sub
        End If
    ...
    Private Sub tipod_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles tipod.SelectedIndexChanged
        llena_combos(tallad, "SELECT DISTINCT TALLA FROM PROD_TER01 WHERE CAJA = '" & cajad.Text & "' AND TIPO = '" & tipod.Text & "' ORDER BY TALLA", "TALLA")
    End Sub
    ...
    Private Sub elimina_linea()
        ...
        'Try
        ok = modifica_caja(co, fg(l, 1), fg(l, 2), fg(l, 3), fg(l, 5) * -1, orden, unid.Text)
        fg.Rows.Remove(l)
        unidn.Text = 0
        cajad.Focus()
        'Catch
    ...
    Private Sub graba_datos()
        ...
        Dim corre_e As Integer
        Dim corre_s As Integer
        Dim docto_e As String
        Dim docto_s As String
        Dim g As New Object
        Dim caja_n As String
        cnn.Open()
        ...
        Try
            ...
            corre_e = g.ToString + 1
            docto_e = Format(corre_e, h)
            ...
            comando.CommandText = strsql
            g = comando.ExecuteScalar()
            corre_s = g.ToString + 1
            docto_s = Format(corre_s, h)
            ...
            For i = 1 To fg.Rows.Count - 1
                ...
                strsql = "UPDATE PROD_TER01 SET UNIDADES = UNIDADES + '" & fg(i, 5) & "' WHERE CAJA = '" & caja_n & "' AND CORTE = '" & fg(i, 6) & "' AND TALLA = '" & fg(i, 3) & "' AND TIPO = '" & fg(i, 2) & "'"
                comando.CommandText = strsql
                afectados = comando.ExecuteNonQuery()
                If afectados = 0 Then
                ...
                strsql = "INSERT INTO PROD_TER02 (TIPO_MOVTO,NO_DOCTO,CAJA,CORTE,TALLA,TIPO,DESCRIPCION,FECHA,UNIDADES,USUARIO) VALUES ('E','" & _
                                                ...
                                                fg(i, 3) & "','" & _
                                                fg(i, 2) & "','" & _
                                                "TRASLADO DE CAJA " & fg(i, 1) & "',GETDATE(),'" & _
                                                fg(i, 5) & "','" & _
                                                obj.usuario & "')"
                comando.CommandText = strsql
                afectados = comando.ExecuteNonQuery()
                ...
                strsql = "UPDATE PROD_TER01 SET UNIDADES = UNIDADES - '" & fg(i, 5) & "' WHERE CAJA = '" & fg(i, 1) & "' AND CORTE = '" & fg(i, 6) & "' AND TALLA = '" & fg(i, 3) & "' AND TIPO = '" & fg(i, 2) & "'"
                comando.CommandText = strsql
                afectados = comando.ExecuteNonQuery()
                ...
                strsql = "INSERT INTO PROD_TER02 (TIPO_MOVTO,NO_DOCTO,CAJA,CORTE,TALLA,TIPO,DESCRIPCION,FECHA,UNIDADES,USUARIO) VALUES ('S','" & _
                                                   ...
                                                   fg(i, 3) & "','" & _
                                                   fg(i, 2) & "','" & _
                                                   "TRASLADO A CAJA " & caja_nueva & "',GETDATE(),'" & _
                                                   fg(i, 5) & "','" & _
                                                   obj.usuario & "')"
                comando.CommandText = strsql
                afectados = comando.ExecuteNonQuery()
                ...
                If InStr(1, caja_nueva, caja_n) = 0 Then
                    caja_nueva = caja_nueva + caja_n
                End If
            Next
            transaccion.Commit()
