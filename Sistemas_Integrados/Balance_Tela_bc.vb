...
Public Class Balance_Tela_bc
    ...
    Dim path As String = "c:\reportes\Balance_tela.xls"
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(Balance_Tela_bc))
        ...
        Me.fg.Size = New System.Drawing.Size(914, 302)
        ...
        Me.Cliente.Size = New System.Drawing.Size(216, 23)
        ...
        Me.ToolTip1.SetToolTip(Me.si, "Calcular Balance de Tela")
        ...
        Me.ToolTip1.SetToolTip(Me.R1, "Otro cliente")
        ...
        Me.ToolTip1.SetToolTip(Me.Impri, "Imprimir")
        ...
        Me.Te.Font = New System.Drawing.Font("Microsoft Sans Serif", 9.75!)
        ...
        Me.ToolTip1.SetToolTip(Me.Ex, "Trasladar a Excel.")
        ...
        Me.nc.Size = New System.Drawing.Size(906, 302)
        ...
        Me.ClientSize = New System.Drawing.Size(1021, 687)
        ...
        Me.Name = "Balance_Tela_bc"
        Me.ShowInTaskbar = False
        Me.StartPosition = System.Windows.Forms.FormStartPosition.CenterScreen
        Me.Text = "Balance Total de Tela "
...
#End Region
    ...
    Private Sub balance_telas_b_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        AddHandler Cliente.KeyPress, AddressOf keypressed1
        Te.Width = Me.Width - 30
        Te.Height = Me.Height - 100
        nc.Width = Te.Width - 10
        nc.Height = Te.Height - 10
        'fg.Height = Te.Height
        setea_grid()
    ...
    Private Sub depura_te()
        Dim i As Integer
        For i = 1 To Te.Rows.Count - 1
            If Te(i, 3) = 0 And Te(i, 4) = 0 And Te(i, 5) = 0 And Te(i, 6) = 0 Then
                Te(i, 0) = "cero"
            End If
        Next
        i = Te.FindRow("cero", 1, 0, False)
        While i > 0
            Te.Rows.Remove(i)
            i = Te.FindRow("cero", 1, 0, False)
        End While
        For i = 3 To 6
            Te.Subtotal(AggregateEnum.Sum, -1, -1, i, "Gran Total -->")
        Next
    End Sub
    ...
    Private Sub consumo_tela()
        ...
        nc.Rows.Count = 1
        nc.Cols.Count = 9
        Dim strSQL As String = "SELECT * FROM CONSUMO_TELA COL WHERE CLIENTE = '" & Cliente.Text & "'"
        llena_tablas(dt, strSQL, cnn)
        lineas = dt.Rows.Count
        nc.Rows.Count = lineas + 1
        lineas = 1
    ...
    Private Sub crea_fg()
        ...
        fg.Cols.Count = 8
        Dim dt As New DataTable()
        Dim strSQL As String = "SELECT ESTILO, COLOR ,CPO_D.CPO ,SUM(TOTAL) AS TOTAL FROM CPO_D  LEFT JOIN CPO ON CPO.CPO = CPO_D.CPO WHERE ESTADO = 'A' AND CLIENTE = '" & Cliente.Text & "' GROUP BY ESTILO,COLOR,CPO_D.CPO"
        llena_tablas(dt, strSQL, cnn)
        lineas = dt.Rows.Count
        fg.Rows.Count = lineas + 1
        lineas = 1
    ...
    Private Sub setea_te()
        ...
        Te.Cols(1).Name = "DESCRIPCION DE LA TELA"
        ...
        Te.Cols(8).Name = "CODIGO"
        For index = 1 To Te.Cols.Count - 1
            Te.Cols(index).Width = 350
            ...
            If index > 2 And index < 8 Then
                ...
                Te.Cols(index).Format = "#####0.00"
                Te.Cols(index).Width = 110
            End If
        Next
        Te.Cols(1).Width = 450
        Te.Cols(0).Visible = False
    ...
    Private Sub tela_inventario()
        ...
        Dim busca As String
        Dim linea As Integer
        strsql = "SELECT CODIGO,COLOR,(SUM(ROLLOS.PESO)/SUM(ROLLOS.YARDAS_I) * SUM(ROLLOS.YARDAS)) AS LIBRAS FROM ROLLOS WHERE ACTIVO IN ('S','0','5') AND YARDAS > 0 AND TIPO = 'TELA' AND CLIENTE = '" & Cliente.Text & "' GROUP BY CODIGO,COLOR"
        llena_tablas(dt, strsql, cnn)
        For Each Me.dr In dt.Rows
            ...
            Else
                linea = Te.Rows.Count
                Te.Rows.Count = linea + 1
                Te(linea, 0) = Trim(dr("CODIGO")) + Trim(dr("COLOR"))
                Te(linea, 1) = Trim(dr("CODIGO"))
                Te(linea, 2) = Trim(dr("COLOR"))
                Te(linea, 3) = 0
                Te(linea, 4) = CDec(dr("LIBRAS"))
            End If
        Next
    ...
    Private Sub tela_textilera()
        ...
        Dim linea As Integer
        Dim dt As New DataTable()
        strsql = "SELECT CODIGO,COLOR,(SUM(ROLLOS.PESO)/SUM(ROLLOS.YARDAS_I) * SUM(ROLLOS.YARDAS)) AS LIBRAS FROM ROLLOS WHERE ACTIVO = '6' AND CODIGO+COLOR IN (SELECT CODIGO+COLOR_TELA FROM CONSUMO_TELA WHERE CLIENTE = '" & Cliente.Text & "') AND YARDAS > 0 AND TIPO = 'TELA' AND CLIENTE = '" & Cliente.Text & "' GROUP BY CODIGO,COLOR"
        llena_tablas(dt, strsql, cnn)
        For Each Me.dr In dt.Rows
            busca = Trim(dr("CODIGO")) + Trim(dr("COLOR"))
            linea = Te.FindRow(busca, 1, 0, True)
    ...
    Private Sub tela_pedida()
        ...
        Dim pedi As Decimal
        Dim dt As New DataTable()
        strsql = "SELECT CODIGO,COLOR, (LIBRAS - RECIBIDAS) AS LIBRAS FROM FPO WHERE CLIENTE = '" & Cliente.Text & "' AND STATUS = 'ABIERTO' AND TIPO = 'TELA' AND RECIBIDAS = 0 ORDER BY CODIGO,COLOR"
        llena_tablas(dt, strsql, cnn)
        For Each Me.dr In dt.Rows
            busca = Trim(dr("CODIGO")) + Trim(dr("COLOR"))
            linea = Te.FindRow(busca, 1, 0, True)
            pedi = CDec(dr("LIBRAS"))
            If busca = "83111920BLACK" Then
                Dim JAPA = 1
            End If
    ...
    Private Sub Te_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Te.DoubleClick
        ...
        Dim codigo As String = Te(linea, 8)
    ...
    Private Sub llena_hoja(ByRef ok As Boolean)
        ...
        Try
            ...
            Catch
            End Try
            Dim sheet As XLSheet = _book.Sheets.Add("Balance Total")
            SaveSheet(_book, Te, sheet, True)
    ...
    Private Sub llena_descripcion()
        ...
        Dim telas As New DataTable()
        Dim dd As DataRow()
        llena_tablas(telas, "SELECT * FROM CODIGO_TELA", cnn)
        For i = 1 To Te.Rows.Count - 1
            codigo = Te(i, 1)
            dd = telas.Select("CODIGO = '" & codigo & "'")
            If dd.Length > 0 Then
                dr = dd(0)
                Te(i, 8) = Te(i, 1)
                Te(i, 1) = dr("DESCRIPCION")
            End If
        Next
    End Sub
