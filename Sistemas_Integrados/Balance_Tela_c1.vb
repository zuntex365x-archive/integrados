...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(Balance_Tela_c1))
        ...
        Me.Name = "Balance_Tela_c1"
        Me.ShowInTaskbar = False
        Me.StartPosition = System.Windows.Forms.FormStartPosition.CenterScreen
        Me.Text = "Balance_Tela_c1 + Balance de Tela por Cliente Codificada"
...
#End Region
    ...
    Private Sub consulta_cortes_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        llena_tablas(cli, "SELECT DISTINCT CPO.CLIENTE, MAXIMO FROM CPO_D LEFT JOIN CPO ON CPO.CPO = CPO_D.CPO LEFT JOIN CLIENTES ON CPO.CLIENTE = CLIENTES.CLIENTE WHERE ESTADO = 'A' ORDER BY CPO.CLIENTE", cnn)
        llena_combos_d(Cliente, cli, "CLIENTE")
        setea_grid()
    ...
    Private Sub llena_dt()
        Dim strsql As String = "SELECT CPO_D.CPO,ESTILO,COLOR,CPO_D.TOTAL AS PEDIDAS ,(SELECT SUM(TOTAL) FROM CORTES_S WHERE CORTES_S.CPO = CPO_D.CPO AND CORTES_S.ESTILO = CPO_D.ESTILO AND CORTES_S.COLOR = CPO_D.COLOR) AS CORTADAS FROM CPO_D LEFT JOIN CPO ON CPO_D.CPO = CPO.CPO WHERE ESTADO = 'A' AND CPO.CLIENTE = '" & Cliente.Text & "' ORDER BY CPO,ESTILO,COLOR"
        llena_tablas(cpo, strsql, cnn)
        strsql = "SELECT ROLLOS.CODIGO,DESCRIPCION,COLOR,0.00 AS L_NEC,SUM((ROLLOS.PESO/YARDAS_I)*ROLLOS.YARDAS) AS INV,'0.00' AS RESERV,'0' AS PEDIDAS,'0.00' AS BALANCE FROM ROLLOS LEFT JOIN CODIGO_TELA ON ROLLOS.CODIGO = CODIGO_TELA.CODIGO WHERE CLIENTE = '" & Cliente.Text & "' AND YARDAS > 0 AND TIPO = 'TELA' AND ACTIVO IN ('S','0','5','6','9') GROUP BY ROLLOS.CODIGO,DESCRIPCION,COLOR"
        llena_tablas(inv, strsql, cnn)
        strsql = "SELECT REQ_D.CORTE,CODIGO,ROLLOS.COLOR,SUM(REQ_D.PESO)AS LIBRAS FROM REQ_D LEFT JOIN CORTES_S ON REQ_D.CORTE = CORTES_S.CORTE LEFT JOIN ROLLOS ON REQ_D.BATCH = ROLLOS.BATCH AND REQ_D.ROLLO = ROLLOS.ROLLO WHERE DESPACHADO = 'N' AND CORTES_S.CLIENTE = '" & Cliente.Text & "' and not exists (select 1 from DMOVTO WHERE DMOVTO.CORTE = REQ_D.CORTE AND DMOVTO.BATCH = REQ_D.BATCH AND DMOVTO.ROLLO = REQ_D.ROLLO) GROUP BY REQ_D.CORTE,CODIGO,ROLLOS.COLOR"
        llena_tablas(res, strsql, cnn)
        strsql = "SELECT CODIGO,COLOR,SUM(LIBRAS) AS LIBRAS FROM FPO WHERE CLIENTE = '" & Cliente.Text & "' AND RECIBIDAS = 0 AND STATUS = 'ABIERTO 'GROUP BY CODIGO,COLOR"
        llena_tablas(fpo, strsql, cnn)
        strsql = "SELECT ESTILO_CPO,COLOR_CPO,CONSUMO_TELA.CODIGO,COLOR_TELA,LIBRAS,DESCRIPCION FROM CONSUMO_TELA LEFT JOIN CODIGO_TELA ON CONSUMO_TELA.CODIGO = CODIGO_TELA.CODIGO WHERE CONSUMO_TELA.ESTILO_CPO + CONSUMO_TELA.COLOR_CPO IN (SELECT ESTILO + COLOR FROM CPO_D LEFT JOIN CPO ON CPO_D.CPO = CPO.CPO WHERE CLIENTE = '" & Cliente.Text & "' AND ESTADO = 'A')"
    ...
    Private Sub llena_grid()
        ...
        For Each dr In inv.Rows
            ...
            End If
            busca_registro(dm, "CODIGO", dr("CODIGO"), "DM", fg(lineas, 8))
            lineas = lineas + 1
        Next
    ...
    Private Sub proceso()
        ...
        For Each dr In cpo.Rows
            ...
            unidades = CInt((pedido * ppc) / 100) - cortadas

            ' Se excluye porcentaje extra para piezas ya confeccionadas
            ' unidades = CInt(((pedido - cortadas) * ppc) / 100)
    ...
    Private Sub Cliente_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cliente.Leave
        Dim dd As DataRow()
        Dim dr As DataRow
        dd = cli.Select("CLIENTE = '" & Cliente.Text & "'")
        If dd.Length > 0 Then
            dr = dd(0)
            ppc = dr("MAXIMO")
            ppc = ppc + 100
    ...
    Private Sub Ex_Click(sender As System.Object, e As System.EventArgs) Handles Ex.Click
        a_excel(fg, "C:\reportes\balance_telas.xls", ok)
    End Sub
End Class
