...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(Empaque_d))
        ...
        Me.S1.Location = New System.Drawing.Point(320, 8)
        Me.S1.Name = "S1"
        ...
        Me.CANCELA.Location = New System.Drawing.Point(320, 8)
        ...
        Me.Text = "Prendas producidas por Corte"
...
#End Region
    ...
    Private Sub setea_grid()
        ...
        fg.Cols(16).Name = "6XL"
        fg.Cols(17).Name = "TOT."
        fg.Cols(18).Name = "FECHA"
        fg.Cols(19).Name = "SECCION"
        fg.Cols(20).Name = "ESCALA"
        ...
        fg.Cols(18).AllowMerging = True
        fg.Cols(19).AllowMerging = True
        fg.Cols(1).Width = 60
        fg.Cols(6).Width = 60
        fg.Cols(17).Width = 60
        fg.Cols(18).DataType = GetType(Date)
        fg.Cols(18).Format = "dd/MM/yyyy"
        fg.Cols(18).Width = 70
        fg.Cols(18).TextAlign = TextAlignEnum.CenterCenter
        fg.Cols(18).TextAlignFixed = TextAlignEnum.CenterCenter
        fg.Cols(19).Width = 110
    End Sub
    ...
    Private Sub llena_grid()
        ...
        ' bugfix 2XL unexistent in scale 00
        Dim strSQL As String = "SELECT PROD_DIARIA.CORTE,CLIENTE,CORTES.CPO,CORTES.ESTILO,CORTES.COLOR,TIPO,FECHA,(case when TALLA = '2XL' AND ESCALA = '00' THEN 'XL2' ELSE TALLA END) TALLA,PRENDAS,SECCION,ESCALA FROM PROD_DIARIA LEFT JOIN CORTES ON PROD_DIARIA.CORTE = CORTES.CORTE LEFT JOIN CPO_D ON CORTES.CPO = CPO_D.CPO AND CORTES.ESTILO = CPO_D.ESTILO AND CORTES.COLOR = CPO_D.COLOR WHERE FECHA BETWEEN '" & fecha & "' AND '" & fecha1 & "' AND PROD_DIARIA.CORTE = CORTES.CORTE"
        llena_tablas(dt, strSQL, cnn)
        strSQL = "SELECT * FROM E_TALLAS"
        ...
        For Each Me.dr In dt.Rows
            ...
            If dr("ESCALA") = "00" Then
                te = Split(tallas, ","c)
            Else
                jj = es.Select("ESCALA = '" & dr("ESCALA") & "'")
                If jj.Length > 0 Then
                    jr = jj(0)
                    te = Split(jr(2) + "|" + jr(3) + "|" + jr(4) + "|" + jr(5) + "|" + jr(6) + "|" + jr(7) + "|" + jr(8) + "|" + jr(9) + "|" + jr(10) + "|" + jr(11) + "|", "|")
                End If
            End If
            ...
            If tipo = 0 Then
                If IsNumeric(fg(si, i + 7)) Then
                    fg(si, i + 7) = fg(si, i + 7) + prendas
                Else
                    fg(si, i + 7) = prendas
                End If
                If IsNumeric(fg(si, 17)) Then
                ...
                fg(si, 19) = dr("SECCION")
            Else
                If IsNumeric(fg(si + 1, i + 7)) Then
                    fg(si + 1, i + 7) = fg(si + 1, i + 7) + prendas
                Else
                    fg(si + 1, i + 7) = prendas
                End If
                If IsNumeric(fg(si + 1, 17)) Then
                    fg(si + 1, 17) = fg(si + 1, 17) + prendas
                Else
                    fg(si + 1, 17) = prendas
                End If
                fg(si + 1, 18) = dr("FECHA")
                fg(si + 1, 19) = dr("SECCION")
            End If
        Next
        subtotales()
    End Sub
