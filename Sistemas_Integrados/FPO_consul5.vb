...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(FPO_consul5))
        ...
        Me.clientes.Location = New System.Drawing.Point(16, 24)
        Me.clientes.Name = "clientes"
        Me.clientes.Size = New System.Drawing.Size(240, 24)
        Me.clientes.TabIndex = 1
        Me.ToolTip1.SetToolTip(Me.clientes, "Elija la Fabric PO  a Consultar .")
        '
        'Button3
        '
...
#End Region
    ...
    Private Sub FPO_consul5_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub llena_yardas()
        ...
        Dim dr As DataRow
        STRSQL = "SELECT DMOVTO.FPO,DMOVTO.TMOVTO, ROLLOS.KNIT, ROLLOS.COLOR, ROLLOS.TIPO, ROLLOS.BATCH,SUM(DMOVTO.YARDAS) AS YARDA, SUM(DMOVTO.LIBRAS) AS LIBRA  FROM DMOVTO, ROLLOS WHERE DMOVTO.FPO + DMOVTO.BATCH IN (SELECT DISTINCT FPO + BATCH FROM ROLLOS WHERE YARDAS > 0 AND TIPO = 'TELA' AND CLIENTE = '" & clientes.Text & "' )  AND  DMOVTO.BATCH = ROLLOS.BATCH AND DMOVTO.ROLLO = ROLLOS.ROLLO GROUP BY DMOVTO.FPO,DMOVTO.TMOVTO, ROLLOS.KNIT, ROLLOS.COLOR, ROLLOS.TIPO, ROLLOS.BATCH ORDER BY DMOVTO.FPO"
        llena_tablas(dt, STRSQL, con)
        For Each dr In dt.Rows
            ...
            TIPO = dr("TIPO")
            LOTE = dr("BATCH")
            lineas = fg.FindRow(fpo + COL + KNIT + TIPO + LOTE, 0, 0, True)
            YARDAS = dr("YARDA")
            LIBRAS = dr("LIBRA")
            MOVTO = dr("TMOVTO")
            If lineas > 0 Then
    ...
    Private Sub Print_fg()
        ...
        fg.PrintGrid("inventario2", PrintGridFlags.ShowPreviewDialog, "Resumen de Movtos.  cliente: " + Trim(clientes.Text) + "  AL  " + fecha + Chr(9) + Chr(9) + "Pagina {0}", "")
    End Sub
    ...
    Private Sub busca_clientes()
        ...
        Dim strSQL As String = "SELECT DISTINCT CLIENTE FROM ROLLOS WHERE YARDAS > 0 AND TIPO = 'TELA' ORDER BY CLIENTE"
        llena_tablas(dt, strSQL, con)
        For Each dr In dt.Rows
    ...
    Private Sub llena_fpo()
        ...
        Dim dt As New DataTable()
        Dim dr As DataRow
        Dim strSQL As String = "SELECT FPO,KNIT, COLOR, TIPO, LOTE,  LIBRAS, RECIBIDAS FROM FPO WHERE FPO IN (SELECT DISTINCT FPO FROM ROLLOS WHERE YARDAS > 0) AND TIPO = 'TELA' AND CLIENTE = '" & clientes.Text & "' ORDER BY FPO,COLOR, KNIT, TIPO, LOTE "
        llena_tablas(dt, strSQL, con)
        For Each dr In dt.Rows
            lineas = lineas + 1
            ...
            TIPO = dr("TIPO")
            LOTE = dr("LOTE")
            fg(lineas, 0) = Trim(dr("FPO")) + Trim(COL) + Trim(KNIT) + Trim(TIPO) + CStr(LOTE)
            fg(lineas, 1) = dr("FPO")
            fg(lineas, 2) = COL
            fg(lineas, 3) = KNIT
            fg(lineas, 4) = TIPO
