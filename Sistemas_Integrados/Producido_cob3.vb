...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(Producido_cob3))
        ...
        Me.Name = "Producido_cob3"
        Me.ShowInTaskbar = False
        Me.StartPosition = System.Windows.Forms.FormStartPosition.CenterScreen
        Me.Text = "Producido vrs Exportado"
...
#End Region
    ...
    Private Sub llena_vales()
        val = New DataTable
        Dim strsql As String = "SELECT  CORTE,SUM(PRENDAS) AS PRENDAS FROM VALES WHERE FECHA " & Mid(rango, 7) + " GROUP BY CORTE"
        llena_tablas(val, strsql, cnn)
    End Sub
    ...
    Private Sub llena_fg()
        ...
        Dim strsql As String = "SELECT PROD_DIARIA.CORTE,CORTES.CPO,CORTES.ESTILO,CORTES.COLOR,TIPO,CORTES.TOTAL,SUM(PRENDAS) AS PRENDAS,SECCION,CLIENTE,CPO_D.ESTADO, FPROD FROM PROD_DIARIA LEFT JOIN CORTES ON PROD_DIARIA.CORTE = CORTES.CORTE  LEFT JOIN CPO_D ON CORTES.CPO = CPO_D.CPO AND CORTES.ESTILO = CPO_D.ESTILO AND CORTES.COLOR = CPO_D.COLOR WHERE  " & rango & "  GROUP BY PROD_DIARIA.CORTE,CORTES.CPO,CORTES.ESTILO,CORTES.COLOR,TIPO,CORTES.TOTAL,SECCION,CLIENTE,CPO_D.ESTADO, FPROD ORDER BY PROD_DIARIA.CORTE,CORTES.CPO,CORTES.ESTILO,CORTES.COLOR ,CPO_D.ESTADO"
        '"SELECT PROD_DIARIA.CORTE,CORTES.CPO,CORTES.ESTILO,CORTES.COLOR,TIPO,TOTAL,SUM(PRENDAS) AS PRENDAS,SECCION,CLIENTE  FROM PROD_DIARIA LEFT JOIN CORTES ON PROD_DIARIA.CORTE = CORTES.CORTE WHERE  " & rango & "  GROUP BY PROD_DIARIA.CORTE,CORTES.CPO,CORTES.ESTILO,CORTES.COLOR,TIPO,TOTAL,SECCION,CLIENTE  ORDER BY PROD_DIARIA.CORTE,CORTES.CPO,CORTES.ESTILO,CORTES.COLOR "
        ...
        For Each dr In dt.Rows
            ...
            If tipo = "0" Then
                fg(f, "c5") = fg(f, "c5") + prendas
            Else
                If (tipo = "3" Or tipo = "8") Then
                    fg(f, "c7") = fg(f, "c7") + prendas
                Else
                    fg(f, "c6") = fg(f, "c6") + prendas
                End If
            End If
        ...
        For i = 1 To fg.Rows.Count - 1
            vales(fg(i, 1), fg(i, "c3"))
            fg(i, "c8") = fg(i, "c3") + fg(i, "c5") + fg(i, "c6") + fg(i, "c7")
            dd = cob.Select("CORTE = '" & fg(i, 1) & "' OR CORTE = 'ST-" & fg(i, 1) & "'")
            If dd.Length > 0 Then
                For Each ds In dd
                    prendas = ds("XS") + ds("S") + ds("M") + ds("L") + ds("XL") + ds("XL2") + ds("XL3") + ds("XL4") + ds("XL5") + ds("XL6")
                    fg(i, "c9") = fg(i, "c9") + prendas
                Next
            End If
            fg(i, "c0") = fg(i, "c1") - fg(i, "c9")
            fg(i, "c4") = fg(i, "c1") - fg(i, "c2")
            If (fg(i, "c4") > 0) And (fg(i, "c2") > 0) Then
                fg.SetCellStyle(i, fg.Cols.IndexOf("c4"), "rojo")
            End If
        Next i
        sub_totales()
    End Sub
    ...
    Private Sub Print_fg()
        ...
        mal = "Cortado Vrs Exportado " + fechad & "  al " & Format(fecha1.Value, "dd/MM/yyyy")
    ...
    Private Sub vales(ByVal corte As String, ByRef uni As Decimal)
        Dim dd As DataRow()
        Dim dj As DataRow
        uni = 0
        dd = Val.select("CORTE = '" & corte & "'")
        If dd.Length > 0 Then
            dj = dd(0)
            uni = dj("PRENDAS")
        End If
    ...
    Private Sub to_excel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles to_excel.Click
        Dim ok As Boolean
        Dim hora As String = "c:\reportes\Producido_vrs_cobrado " & Format(Now, "yyyy_MMM_dd HH_mm") & ".xls"
        a_excel(fg, hora, ok)
        Close()
    End Sub
End Class
