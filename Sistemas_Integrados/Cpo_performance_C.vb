...
Public Class Cpo_performance_C
...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(Cpo_performance_C))
...
#End Region
    ...
    Private Sub llena_fg()
        ...
        Dim fen As Date
        Dim por As Decimal
        Dim strsql As String = "SELECT CPO_D.CPO,CPO_D.ESTILO,CPO_D.COLOR,CLIENTE,TOTAL,CPO_D.F_ENTREGA,EXP_REV,CPO_D.F_INGRESO  FROM CPO_D LEFT JOIN CPO ON CPO_D.CPO = CPO.CPO LEFT JOIN WIP0 ON CPO_D.CPO = WIP0.CPO AND CPO_D.ESTILO = WIP0.ESTILO AND CPO_D.COLOR = WIP0.COLOR AND TALLA = 'TOTAL' WHERE CLIENTE = '" & C4.Text & "' AND ( CPO_D.F_ENTREGA BETWEEN '" & Format(Fecha_inicial.Value, "yyyy-MM-dd") & "' AND '" & Format(Fecha_final.Value, "yyyy-MM-dd") & "') ORDER BY CPO_D.CPO"
        strsql = "SELECT CPO_D.CPO,CPO_D.ESTILO,CPO_D.COLOR,CLIENTE,TOTAL,CPO_D.F_ENTREGA,EXP_REV,CPO_D.F_INGRESO FROM CPO_D LEFT JOIN CPO ON CPO_D.CPO = CPO.CPO LEFT JOIN WIP0 ON CPO_D.CPO = WIP0.CPO AND CPO_D.ESTILO = WIP0.ESTILO AND CPO_D.COLOR = WIP0.COLOR AND TALLA = 'TOTAL' WHERE ( CPO_D.F_ENTREGA BETWEEN '" & Format(Fecha_inicial.Value, "yyyy-MM-dd") & "' AND '" & Format(Fecha_final.Value, "yyyy-MM-dd") & "') ORDER BY CPO_D.CPO"
        ...
        For Each Me.dr In dt.Rows
            ...
            If dd.Length > 0 Then
                For Each dj In dd
                    fg(l, 6) = fg(l, 6) + dj("EXPORTADO")
                    fg(l, 8) = dj("FECHA")
                Next
            End If
           
            por = (fg(l, 6) * 100) / fg(l, 5)
            If por < 94 Then
                fg(l, 9) = Today    ' Pedido no completado
            End If
    ...
    Private Sub C4_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles C4.SelectedIndexChanged
        ...
        llena_tablas(dx, "SELECT CPO,ESTILO,COLOR,CLIENTE,SUM(XS+S+M+L+XL+XL2+XL3+XL4+XL5+XL6) AS EXPORTADO, FECHA  FROM PACKING_D WHERE TIPO = 'Pri' GROUP BY CPO,ESTILO,COLOR ,CLIENTE,FECHA ORDER BY CPO,ESTILO,COLOR,FECHA ", cnn)
    End Sub
